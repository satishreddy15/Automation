...
test('GuestDashboardVisit', async ({ page }) => { 
  ...
  await page.locator('#mat-menu-panel-3').getByText('Family Guest visitor type').click();
  await page.getByText('Phone/Name').click();
  await page.getByRole('textbox', { name: 'searchVisitor' }).fill('mounika');
  await page.getByRole('button', { name: 'SEARCH' }).click();
await page.getByRole('row', { name: 'Mounika Resident 630-685-7535' }).getByLabel('', { exact: true }).check();
await page.locator('.mat-mdc-select-placeholder').first().click();
await page.getByText('Visit Resident type').click();
await page.getByRole('row', { name: 'Abhi Q Test Assisted Living' }).getByLabel('', { exact: true }).check();
...
await page.locator('//mat-row[.//text()[contains(.,"Mounika")]]//mat-checkbox//input').first().check();
