...
test("Invalid Credentials", async ({ page }) => {
  ...
  await page.getByRole("checkbox", { name: "Remember me" }).check();
  await expect(page.getByText('SIGN IN', { exact: true })).toBeEnabled({ timeout: 6000 });
  await page.getByText('SIGN IN', { exact: true }).click();
  await expect(page.getByText('Please enter valid email and password.')).toBeVisible({ timeout: 7000 });
...
test("Logout Functionality", async ({ page }) => {
  ...
  await expect(page.getByText('SIGN IN', { exact: true })).toBeEnabled();
  await page.getByText('SIGN IN', { exact: true }).click();
  await expect(page.locator('span:has-text("account_circle")')).toBeVisible({ timeout: 7000 });
